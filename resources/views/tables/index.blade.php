...
@section('title', 'Manage Tables - Tasty Station')
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-gray-900">Manage Tables</h2>
                    <a href="{{ route('tables.create') }}" class="px-4 py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600">
                        <i data-lucide="plus" class="w-4 h-4 inline"></i> Add New Table
                    </a>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach($tables as $table)
                    <div class="border rounded-lg p-4 {{ $table->status === 'occupied' ? 'bg-red-50 border-red-200' : ($table->status === 'reserved' ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200') }}">
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                <h3 class="text-xl font-bold text-gray-900">Table {{ $table->table_number }}</h3>
                                <p class="text-sm text-gray-600">Capacity: {{ $table->capacity }} people</p>
                            </div>
                            <span class="px-2 py-1 text-xs font-semibold rounded {{ $table->status === 'occupied' ? 'bg-red-500 text-white' : ($table->status === 'reserved' ? 'bg-yellow-500 text-white' : 'bg-green-500 text-white') }}">
                                {{ ucfirst($table->status) }}
                            </span>
                        </div>
                        
                        @if($table->orders_count > 0)
                        <p class="text-sm text-gray-600 mb-3">Active Orders: {{ $table->orders_count }}</p>
                        @endif

                        <div class="flex space-x-2">
                            <a href="{{ route('tables.edit', $table) }}" class="flex-1 px-3 py-2 bg-blue-500 text-white text-sm rounded hover:bg-blue-600 text-center">
                                Edit
                            </a>
                            <form action="{{ route('tables.destroy', $table) }}" method="POST" class="flex-1" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full px-3 py-2 bg-red-500 text-white text-sm rounded hover:bg-red-600">
                                    Delete
                                </button>
                            </form>
                        </div>
