<!-- resources/views/admin/edit-food.blade.php -->
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Edit Food - OrderNow!</title>
...
<body class="bg-gray-50">
    <div class="max-w-4xl mx-auto py-8 px-4">
        ...
        <div class="bg-white rounded-lg shadow-sm p-8">
            <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Food Item</h1>
            ...
            <form method="POST" action="{{ route('admin.foods.update', $food->id) }}" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $food->name) }}" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-red-500 @error('name') border-red-500 @enderror">
                        @error('name')
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="price" value="{{ old('price', $food->price) }}" required step="0.01" min="0" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-red-500 @error('price') border-red-500 @enderror">
                            @error('price')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        @if($food->image)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $food->image) }}" alt="{{ $food->name }}" class="w-32 h-32 object-cover rounded-lg">
                        </div>
                        @endif
                ...
                <div class="flex justify-end space-x-3 mt-8">
                    ...
                    <button type="submit" class="px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">Update Food Item</button>
                </div>
            </form>
        </div>
    </div>
